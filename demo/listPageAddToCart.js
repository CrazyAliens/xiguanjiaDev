...
/****************************************html******************************************************/
    $(document.body).append(pophtml);//向文档插入dom节点

    $('.att-popup-cart').on('click', function (event) {//购物车点击事件
        if ($(event.target).is('.att-popup-close')) {//判断委托对象是否是'.att-popup-close'关闭按钮
...
function AddToCart(obj) {
    ...
    if (isNaN(productId) || productId <= 0) {//如果产品ID不存在 return
        return false;
    }
    toCartStoreId = parseInt($(obj).attr("StoreId"));//获取StoreId
    if (isNaN(toCartStoreId) || toCartStoreId <= 0) {
        toCartStoreId = parseInt(getParam("StoreId"));
        if (isNaN(toCartStoreId) || toCartStoreId <= 0) {
            toCartStoreId = 0;
